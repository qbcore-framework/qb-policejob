...
local function UpdateBlips()
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "police" or v.PlayerData.job.type == "leo" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.type == "leo" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
        TriggerClientEvent("police:client:GetEscorted", EscortPlayer.PlayerData.source, Player.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel) -- Send to Garages
    local src = source
...
RegisterNetEvent('police:server:KidnapPlayer', function(playerId)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"] then  --- stevie was here
...
RegisterNetEvent('police:server:TakeOutImpound', function(plate, garage) -- Move to garages!
    local src = source
    local playerPed = GetPlayerPed(src)
    local playerCoords = GetEntityCoords(playerPed)
...
-- Deprecated callbacks due to qb-jobs
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.type == "leo") and Player.PlayerData.job.grade.level >= Config.LicenseRank or Player.PlayerData.job.type == "leo" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args) -- potential move to legal job
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
-- Deprecated thrads due to qb-jobs
CreateThread(function()
    if QBCore.Shared.QBJobsStatus then return end
    while true do
        Wait(5000)
