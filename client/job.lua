...
RegisterNetEvent('police:client:createBlip')
AddEventHandler('police:client:createBlip', function(type, coords, name, message, id, source)
	...
	if ((PlayerJob.name == "police" or PlayerJob.name == "ambulance") and PlayerJob.onduty) or (source == GetPlayerServerId(PlayerId())) then
		...
		if type == "police" then
            TriggerEvent('chat:addMessage', {template = '<div class="chat-message" style="background-color: rgba(59, 160, 255, 0.75);"><b>Emergency Call 911 #' .. id .. ' | ' .. name .. '</b> ' .. message .. '</div>'})
		elseif type == "ems" then
			TriggerEvent('chat:addMessage', {template = '<div class="chat-message" style="background-color: rgba(255, 59, 62, 0.75);"><b>Emergency Call 977 #' .. id .. ' | ' .. name .. '</b> ' .. message .. '</div>'})
		end
...
RegisterNetEvent('police:client:reply')
AddEventHandler('police:client:reply', function(type, id, message,officer, source)
	if ((PlayerJob.name == "police" or PlayerJob.name == "ambulance") and PlayerJob.onduty) or (source == GetPlayerServerId(PlayerId())) then
		TriggerEvent('chat:addMessage', {template = '<div class="chat-message server"><b>Emergency Call ' .. (type == 'police' and '911' or '977') .. 'r #' .. id .. ' | ' .. officer .. '</b> ' .. message .. '</div>'})
		TriggerEvent("InteractSound_CL:PlayOnOne", "demo", 0.4)
	end
end)

RegisterNetEvent('police:client:called')
AddEventHandler('police:client:called', function()
